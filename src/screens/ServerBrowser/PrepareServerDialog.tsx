...
import { errorMessage, minecraftApi } from "@/services/tauri";
    ...
    <Dialog open={target !== null} onOpenChange={(open) => !prepare.isPending && onOpenChange(open)}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Une instance Fabric dédiée est créée : bonne version, mods choisis, et le jeu se connecte directement au
            serveur.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button className="bg-gradient-brand shadow-glow gap-1.5" disabled={!ready} onClick={() => prepare.mutate()}>
            {prepare.isPending && <Loader2 className="animate-spin" aria-hidden="true" />}
            {prepare.isPending ? "Installation des mods…" : "Préparer l'instance"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
